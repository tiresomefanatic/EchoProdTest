xx
<template>
  <footer class="footer">
    <div class="footer-inner">
      <h2 class="footer-title">Help and support</h2>

      <hr class="footer-divider" />

      <div class="footer-grid">
        <div class="feedback-section">
          <h3 class="section-title">Did you find what you needed?</h3>

          <div class="button-group">
            <button class="feedback-button">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="25"
                viewBox="0 0 24 25"
                fill="none"
              >
                <mask
                  id="mask0_40000795_16378"
                  style="mask-type: alpha"
                  maskUnits="userSpaceOnUse"
                  x="0"
                  y="0"
                  width="24"
                  height="25"
                >
                  <rect y="0.806641" width="24" height="24" fill="#D9D9D9" />
                </mask>
                <g mask="url(#mask0_40000795_16378)">
                  <path
                    d="M18 21.8066H7V8.80664L14 1.80664L15.25 3.05664C15.3667 3.17331 15.4625 3.33164 15.5375 3.53164C15.6125 3.73164 15.65 3.92331 15.65 4.10664V4.45664L14.55 8.80664H21C21.5333 8.80664 22 9.00664 22.4 9.40664C22.8 9.80664 23 10.2733 23 10.8066V12.8066C23 12.9233 22.9833 13.0483 22.95 13.1816C22.9167 13.315 22.8833 13.44 22.85 13.5566L19.85 20.6066C19.7 20.94 19.45 21.2233 19.1 21.4566C18.75 21.69 18.3833 21.8066 18 21.8066ZM9 19.8066H18L21 12.8066V10.8066H12L13.35 5.30664L9 9.65664V19.8066ZM7 8.80664V10.8066H4V19.8066H7V21.8066H2V8.80664H7Z"
                    fill="#1C1B1F"
                  />
                </g>
              </svg>
              <span>Yes</span>
            </button>
            <button class="feedback-button">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="25"
                viewBox="0 0 24 25"
                fill="none"
              >
                <mask
                  id="mask0_40000795_16383"
                  style="mask-type: alpha"
                  maskUnits="userSpaceOnUse"
                  x="0"
                  y="0"
                  width="24"
                  height="25"
                >
                  <rect y="0.806641" width="24" height="24" fill="#D9D9D9" />
                </mask>
                <g mask="url(#mask0_40000795_16383)">
                  <path
                    d="M6 3.80664H17V16.8066L10 23.8066L8.75 22.5566C8.63333 22.44 8.5375 22.2816 8.4625 22.0816C8.3875 21.8816 8.35 21.69 8.35 21.5066V21.1566L9.45 16.8066H3C2.46667 16.8066 2 16.6066 1.6 16.2066C1.2 15.8066 1 15.34 1 14.8066V12.8066C1 12.69 1.01667 12.565 1.05 12.4316C1.08333 12.2983 1.11667 12.1733 1.15 12.0566L4.15 5.00664C4.3 4.67331 4.55 4.38997 4.9 4.15664C5.25 3.92331 5.61667 3.80664 6 3.80664ZM15 5.80664H6L3 12.8066V14.8066H12L10.65 20.3066L15 15.9566V5.80664ZM17 16.8066V14.8066H20V5.80664H17V3.80664H22V16.8066H17Z"
                    fill="#1C1B1F"
                  />
                </g>
              </svg>
              <span>No</span>
            </button>
          </div>
        </div>

        <div class="contribution-section">
          <h3 class="section-title">Help us make these docs great!</h3>
          <p class="section-text">
            All ECHO docs are open source. See something that's wrong or
            unclear? Submit a pull request
          </p>

          <button class="contribution-button">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="25"
              viewBox="0 0 24 25"
              fill="none"
            >
              <mask
                id="mask0_40000795_16392"
                style="mask-type: alpha"
                maskUnits="userSpaceOnUse"
                x="0"
                y="0"
                width="24"
                height="25"
              >
                <rect
                  width="24"
                  height="24"
                  transform="matrix(-1 0 0 1 24 0.806641)"
                  fill="#D9D9D9"
                />
              </mask>
              <g mask="url(#mask0_40000795_16392)">
                <path
                  d="M17 22.8066C17.8333 22.8066 18.5417 22.515 19.125 21.9316C19.7083 21.3483 20 20.64 20 19.8066C20 19.1566 19.8125 18.5733 19.4375 18.0566C19.0625 17.54 18.5833 17.1816 18 16.9816V8.63164C18.5833 8.43164 19.0625 8.07331 19.4375 7.55664C19.8125 7.03997 20 6.45664 20 5.80664C20 4.97331 19.7083 4.26497 19.125 3.68164C18.5417 3.09831 17.8333 2.80664 17 2.80664C16.1667 2.80664 15.4583 3.09831 14.875 3.68164C14.2917 4.26497 14 4.97331 14 5.80664C14 6.45664 14.1875 7.03997 14.5625 7.55664C14.9375 8.07331 15.4167 8.43164 16 8.63164V8.80664C16 9.63997 15.7083 10.3483 15.125 10.9316C14.5417 11.515 13.8333 11.8066 13 11.8066H11C9.61667 11.8066 8.4375 12.2941 7.4625 13.2691C6.4875 14.2441 6 15.4233 6 16.8066V16.9816C5.41667 17.1816 4.9375 17.54 4.5625 18.0566C4.1875 18.5733 4 19.1566 4 19.8066C4 20.64 4.29167 21.3483 4.875 21.9316C5.45833 22.515 6.16667 22.8066 7 22.8066C7.83333 22.8066 8.54167 22.515 9.125 21.9316C9.70833 21.3483 10 20.64 10 19.8066C10 19.1566 9.8125 18.5733 9.4375 18.0566C9.0625 17.54 8.58333 17.1816 8 16.9816V16.8066C8 15.9733 8.29167 15.265 8.875 14.6816C9.45833 14.0983 10.1667 13.8066 11 13.8066H13C13.5667 13.8066 14.1042 13.7191 14.6125 13.5441C15.1208 13.3691 15.5833 13.1233 16 12.8066V16.9816C15.4167 17.1816 14.9375 17.54 14.5625 18.0566C14.1875 18.5733 14 19.1566 14 19.8066C14 20.64 14.2917 21.3483 14.875 21.9316C15.4583 22.515 16.1667 22.8066 17 22.8066Z"
                  fill="#1C1B1F"
                />
              </g>
            </svg>
            <span>Make a contribution</span>
          </button>
          <a href="#" class="learn-link">Learn how to contribute</a>
        </div>

        <div class="help-section">
          <h3 class="section-title">Still need help?</h3>
          <div class="help-links">
            <a href="#" class="help-link">Provide ECHO feedback</a>
            <a href="#" class="help-link">Contact support</a>
          </div>
        </div>
      </div>
    </div>
  </footer>
</template>

<script setup>
// No additional script needed for now
</script>

<style scoped>
.footer {
  margin-top: 64px;
  width: 100%;
  display: flex;
  justify-content: center;
  box-sizing: border-box;
  overflow-x: hidden;
}

@media screen and (min-width: 1025px) {
  .footer {
    margin-left: 40px;
  }
}
@media screen and (min-width: 1024px) and (max-width: 1379px) {
  .footer-inner {
    max-width: 745px;
  }
}

.footer-inner {
  width: 100%;
  box-sizing: border-box;
}

.footer-title {
  color: #000;
  font-family: var(--font-primary);
  font-size: 20px;
  font-style: normal;
  font-weight: 700;
  line-height: 120%;
  margin: 0 0 16px;
}

.footer-divider {
  margin-bottom: 32px;
  margin-top: 8px;
  color: #d9d9d9;
}

@media screen and (min-width: 1380px) {
  .footer-inner {
    max-width: calc(100% - 130px);
  }
}

.footer-grid {
  display: grid;
  grid-template-columns: repeat(3, auto);
  gap: 32px;
}

.section-title {
  color: #000;
  font-family: var(--font-primary);
  font-size: 16px;
  font-style: normal;
  font-weight: 600;
  line-height: 140%;
  margin: 0 0 16px;
}

.button-group {
  display: flex;
  gap: 12px;
}

.feedback-button {
  display: flex;
  width: 88px;
  padding: 8px 16px;
  border: 1px solid #d9d9d9;
  border-radius: 8px;
  background: white;
  cursor: pointer;
  justify-content: center;
  align-items: center;
  gap: 4px;
}

.feedback-button span {
  color: #000;
  font-family: var(--font-primary);
  font-size: 16px;
  font-weight: 600;
  line-height: 140%;
}

.section-text {
  color: #1e1e1e;
  font-family: var(--font-primary);
  font-size: 14px;
  font-style: normal;
  font-weight: 400;
  line-height: 140%;
  margin: 0 0 16px;
}

.contribution-button {
  display: flex;
  padding: 8px 16px;
  justify-content: center;
  align-items: center;
  gap: 4px;
  border-radius: 8px;
  border: 1px solid #d4d4d4;
  background: white;
  cursor: pointer;
}

.contribution-button span {
  color: #000;
  font-family: var(--font-primary);
  font-size: 16px;
  font-weight: 600;
  line-height: 140%;
}

.learn-link {
  color: #518eea;
  font-family: var(--font-primary);
  font-size: 16px;
  font-style: normal;
  font-weight: 530;
  line-height: 120%;
  letter-spacing: -0.32px;
  text-decoration: underline;
  display: block;
  margin-top: 4px;
}

.help-section {
  margin-top: 0;
}

.help-links {
  display: flex;
  flex-direction: column;
  gap: 8px;
}

.help-link {
  color: #518eea;
  font-family: var(--font-primary);
  font-size: 16px;
  font-style: normal;
  font-weight: 530;
  line-height: 120%;
  letter-spacing: -0.32px;
  text-decoration: none;
}

/* Media Queries to match text container's alignment */
@media screen and (min-width: 1380px) {
  .feedback-section {
    text-align: left;
  }

  .contribution-section {
    text-align: left;
    margin: 0 auto;
    max-width: 334px;
  }

  .help-section {
    text-align: left;
  }
}

@media screen and (min-width: 1025px) and (max-width: 1379px) {
}

@media screen and (min-width: 768px) and (max-width: 1068px) {
  .footer-grid {
    grid-template-columns: repeat(2, 1fr);
  }

  .help-section {
    grid-column: 1 / -1;
  }
}

@media screen and (max-width: 767px) {
  .footer-inner {
    padding: 0 0px;
    width: 100%;
  }

  .footer-grid {
    display: flex;
    flex-direction: column;
    gap: 32px;
  }

  .footer-grid .feedback-section {
    margin-bottom: 0;
  }

  .footer-grid .contribution-section,
  .footer-grid .help-section {
    grid-column: unset;
    margin-top: 32px;
  }
}
</style>
