...
// Computed properties
const isVisible = computed(() => sidebarEditorStore.showNewFolderModal);
const parentPath = computed(() => sidebarEditorStore.newItemParentPath);
const displayPath = computed(() => {
  const path = parentPath.value === '/' 
    ? '/' 
    : `${parentPath.value}/`;
  return `${path}${folderName.value.trim() || 'category-name'}`;
});
...
const handleCreate = async () => {
  ...
  sidebarEditorStore.newItemName = folderName.value;
  await sidebarEditorStore.createNewFolder();
};
