...
export default defineEventHandler(async (event) => {
  try {
    ...
    for await (const obj of stream) {
      ...
      const url = await minioClient.presignedGetObject(minioBucket, obj.name, 24 * 60 * 60); // 24 hour expiry
    ...
    objects.sort((a, b) => new Date(b.uploadDate).getTime() - new Date(a.uploadDate).getTime());
