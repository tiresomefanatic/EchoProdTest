...
export function createMinioClient(forcePublic = false): Minio.Client {
  ...
  console.log(`Creating MinIO client with endpoint: ${minioHost}:${minioPort} (SSL: ${useSSL})`);
 ...
 */
export async function ensureBucketExists(client: Minio.Client, bucketName: string): Promise<void> {
  try {
    ...
    console.log(`Setting public read/write policy for bucket '${bucketName}'...`);
    const policy = {
      Version: '2012-10-17',
      Statement: [
        {
